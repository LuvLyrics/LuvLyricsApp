...
export const parseTimestampedLyrics = (rawText: string): LyricLine[] => {
  ...
  if (typeof __DEV__ !== 'undefined' && __DEV__) console.log(`[Parser] Raw text length: ${rawText.length}, Lines: ${lines.length}`);
  if (lines.length > 0) {
      if (typeof __DEV__ !== 'undefined' && __DEV__) console.log(`[Parser] First line: "${lines[0]}"`);
  }
  ...
  if (!hasTimestamps) {
    if (typeof __DEV__ !== 'undefined' && __DEV__) console.log('[Parser] No timestamps detected in text');
