...
const log = (msg: string, data?: any) => {
  if (typeof __DEV__ !== 'undefined' && __DEV__) console.log(`${LOG_PREFIX} ${msg}`, data ?? '');
};
  ...
  try {
    ...
    if (song) {
      ...
      if (song.audioUri && song.audioUri.includes(FileSystem.documentDirectory!)) {
        try {
          if (typeof __DEV__ !== 'undefined' && __DEV__) console.log(`[QUERIES] Deleting physical audio file: ${song.audioUri}`);
          await FileSystem.deleteAsync(song.audioUri, { idempotent: true });
        } catch {
          if (typeof __DEV__ !== 'undefined' && __DEV__) console.warn('[QUERIES] Failed to delete audio file, it might not exist');
        }
      }
      
      if (song.coverImageUri && (song.coverImageUri.includes(FileSystem.documentDirectory!) || song.coverImageUri.includes('file:///'))) {
        try {
          if (typeof __DEV__ !== 'undefined' && __DEV__) console.log(`[QUERIES] Deleting physical cover image: ${song.coverImageUri}`);
          await FileSystem.deleteAsync(song.coverImageUri, { idempotent: true });
        } catch {
          if (typeof __DEV__ !== 'undefined' && __DEV__) console.warn('[QUERIES] Failed to delete cover image');
        }
      }
    }
    ...
    if (typeof __DEV__ !== 'undefined' && __DEV__) console.error('[QUERIES] deleteSong failed during file or DB cleanup:', error);
    throw error;
  }
};
