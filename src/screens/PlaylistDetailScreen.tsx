...
export const PlaylistDetailScreen: React.FC = () => {
  ...
  useEffect(() => {
     const loadSort = async () => {
         try {
             ...
             if (typeof __DEV__ !== 'undefined' && __DEV__) console.log('Failed to load sort settings', e);
         }
     };
     loadSort();
  ...
  const saveSort = async (option: SortOption, direction: SortDirection) => {
      try {
          ...
          if (typeof __DEV__ !== 'undefined' && __DEV__) console.error('Failed to save sort settings', e);
      }
  };
      ...
      if (activeIsPlaying && currentSong && player) {
        ...
        if (needsLoad) {
           if (currentSong.audioUri) {
               try {
                   if (typeof __DEV__ !== 'undefined' && __DEV__) console.log(`[InlinePlayer] Loading audio for: ${currentSong.title}`);
                   ...
                   if (typeof __DEV__ !== 'undefined' && __DEV__) console.log('[InlinePlayer] Load failed', e);
               }
           }
        }
      }
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      if (typeof __DEV__ !== 'undefined' && __DEV__) console.error('Failed to load playlist', e);
    } finally {
      setLoading(false);
    }
  ...
  const handleDragEnd = useCallback(async ({ data }: { data: Song[] }) => {
      ...
      try {
          ...
          if (typeof __DEV__ !== 'undefined' && __DEV__) console.error('Reorder failed', e);
      }
