    ...
    try {
      while (true) {
        ...
        try {
          ...
          if (typeof __DEV__ !== 'undefined' && __DEV__) console.error(`[ScanQueue] Error processing "${nextJob.title}":`, error);
          updateJob(prev => ({
            status: 'failed' as const,
      ...
      if (typeof __DEV__ !== 'undefined' && __DEV__) console.error('[ScanQueue] Queue processor error:', error);
    } finally {
