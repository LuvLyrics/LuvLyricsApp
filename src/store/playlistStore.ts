...
export const usePlaylistStore = create<PlaylistState>((set, get) => ({
  ...
  fetchPlaylists: async () => {
    ...
    try {
      ...
      if (typeof __DEV__ !== 'undefined' && __DEV__) console.log(`[PLAYLIST_STORE] Loaded ${playlists.length} playlists, ${likedIds.size} liked songs`);
    } catch (error) {
      if (typeof __DEV__ !== 'undefined' && __DEV__) console.error('[PLAYLIST_STORE] Fetch error:', error);
      set({ 
  ...
  createPlaylist: async (name, description, coverUri) => {
    ...
    try {
      ...
      if (typeof __DEV__ !== 'undefined' && __DEV__) console.error('[PLAYLIST_STORE] Create error:', error);
  ...
  addSongToPlaylist: async (playlistId, songId) => {
    try {
      ...
      if (typeof __DEV__ !== 'undefined' && __DEV__) console.error('[PLAYLIST_STORE] Add song error:', error);
  ...
  addSongsToPlaylist: async (playlistId, songIds) => {
    try {
        ...
        if (typeof __DEV__ !== 'undefined' && __DEV__) console.error('[PLAYLIST_STORE] Batch add error:', error);
        throw error;
    }
  },
  ...
  removeSongFromPlaylist: async (playlistId, songId) => {
    try {
      ...
      if (playerState.currentPlaylistId === playlistId) {
        ...
        if (typeof __DEV__ !== 'undefined' && __DEV__) console.log(`[PLAYLIST_STORE] Removed ${songId} from active playlist queue`);
      }
      ...
      if (typeof __DEV__ !== 'undefined' && __DEV__) console.error('[PLAYLIST_STORE] Remove song error:', error);
      // Revert optimistic update if needed
      if (playlistId === get().defaultPlaylistId) {
         set(state => {
          const newSet = new Set(state.likedSongIds);
  ...
  toggleLiked: async (songId) => {
    ...
    if (!defaultPlaylistId) {
      if (typeof __DEV__ !== 'undefined' && __DEV__) console.error('[PLAYLIST_STORE] No default playlist found!');
      return;
    }
    ...
    try {
      if (typeof __DEV__ !== 'undefined' && __DEV__) console.log(`[PLAYLIST_STORE] Toggling like for ${songId}. Currently liked: ${isLiked}`);
      ...
      if (typeof __DEV__ !== 'undefined' && __DEV__) console.error('[PLAYLIST_STORE] Toggle like failed:', error);
