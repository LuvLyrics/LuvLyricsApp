...
export const SongVersionSearchModal: React.FC<Props> = ({ visible, targetSong, onClose, onSuccess }) => {
  ...
  const deleteSongFile = async (uri: string) => {
      try {
          ...
          if (typeof __DEV__ !== 'undefined' && __DEV__) console.warn('Failed to delete old audio:', e);
      }
  };
  ...
  const handleReplace = async (newVersion: UnifiedSong) => {
      ...
      try {
          ...
          if (!downloadUrl) {
              if (typeof __DEV__ !== 'undefined' && __DEV__) console.warn('[VersionReplace] No direct URL found. Result:', newVersion);
