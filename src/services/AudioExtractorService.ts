...
class AudioExtractorService {
    ...
    async getAudioFormats(youtubeUrl: string): Promise<AudioFormat[]> {
        try {
            if (typeof __DEV__ !== 'undefined' && __DEV__) console.log(`[AudioExtractor] Fetching info for: ${youtubeUrl}`);
            ...
            if (typeof __DEV__ !== 'undefined' && __DEV__) console.log(`[AudioExtractor] Raw formats found: ${info.formats.length}`);
            
            // Filter for Audio Only
            let audioFormats = ytdl.filterFormats(info.formats, 'audioonly');
            if (typeof __DEV__ !== 'undefined' && __DEV__) console.log(`[AudioExtractor] Audio-only formats: ${audioFormats.length}`);
            
            // RELAXED FILTER: If no audio-only, try filters that contain audio (audioandvideo)
            if (!audioFormats.length) {
                if (typeof __DEV__ !== 'undefined' && __DEV__) console.warn('[AudioExtractor] No audio-only formats, trying to find ANY format with audio (audioandvideo)...');
                audioFormats = ytdl.filterFormats(info.formats, 'audioandvideo');
            }

            if (typeof __DEV__ !== 'undefined' && __DEV__) console.log(`[AudioExtractor] Final formats count: ${audioFormats.length}`);

            if (!audioFormats.length) {
                if (typeof __DEV__ !== 'undefined' && __DEV__) console.warn('[AudioExtractor] No audio formats found (Strict or Relaxed)');
                return [];
            }
            ...
            if (typeof __DEV__ !== 'undefined' && __DEV__) console.error('[AudioExtractor] Error:', error);
