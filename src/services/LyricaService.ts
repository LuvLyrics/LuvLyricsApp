...
class LyricaService {
  async fetchLyrics(song: string, artist: string, syncedOnly: boolean = false, duration?: number): Promise<LyricaResult | null> {
    try {
      ...
      if (typeof __DEV__ !== 'undefined' && __DEV__) console.log('[Lyrica] Cleaned - Artist:', cleanArtist, 'Song:', cleanSong, 'Duration:', duration);
      ...
      if (syncedOnly) {
        ...
        if (typeof __DEV__ !== 'undefined' && __DEV__) console.log('[Lyrica] Synced-only mode active');
      }
      ...
      for (const strategy of strategies) {
        ...
        if (typeof __DEV__ !== 'undefined' && __DEV__) console.log(`[Lyrica] Trying ${strategy.label}`);
        
        const result = await this.executeFetch(url, strategy.label);
        if (result) return result;
      }
      
      if (typeof __DEV__ !== 'undefined' && __DEV__) console.log('[Lyrica] All strategies exhausted');
      return null;
    } catch (error) {
      if (typeof __DEV__ !== 'undefined' && __DEV__) console.error('[Lyrica] Fetch error:', error);
      throw error;
    }
  }
  ...
  private async executeFetch(url: string, label: string): Promise<LyricaResult | null> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (typeof __DEV__ !== 'undefined' && __DEV__) console.log(`[Lyrica] ${label} HTTP ${response.status}:`, truncatedError);
      ...
      if (data.status === 'success' && data.data) {
        ...
        if (typeof finalLyrics === 'string' && (finalLyrics.includes('<div') || finalLyrics.includes('<html') || finalLyrics.includes('<!DOCTYPE'))) {
            if (typeof __DEV__ !== 'undefined' && __DEV__) console.warn(`[Lyrica] ${label} returned HTML instead of lyrics, rejecting.`);
            return null;
        }
      ...
      if (err.message === 'TIMEOUT' || err.name === 'AbortError') {
         if (typeof __DEV__ !== 'undefined' && __DEV__) console.warn(`[Lyrica] ${label} timed out safely (45s limit).`);
         throw new Error('Lyrics request timed out');
      }

      if (typeof __DEV__ !== 'undefined' && __DEV__) console.log(`[Lyrica] ${label} failed:`, err.message || 'Unknown Network Error');
