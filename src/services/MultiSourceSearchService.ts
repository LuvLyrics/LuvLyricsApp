...
async function searchGaana(query: string): Promise<UnifiedSong[]> {
  try {
    if (typeof __DEV__ !== 'undefined' && __DEV__) console.log(`[Gaana] Searching (Fallback): ${query}`);
    ...
    if (typeof __DEV__ !== 'undefined' && __DEV__) console.warn(`[Gaana] Search failed: ${getErrorMessage(error)}`);
    return [];
  }
}
...
async function searchSaavn(query: string): Promise<UnifiedSong[]> {
  try {
    if (typeof __DEV__ !== 'undefined' && __DEV__) console.log(`[Saavn] Searching: ${query}`);
    ...
    if (!response.ok) {
        if (typeof __DEV__ !== 'undefined' && __DEV__) console.warn(`[Saavn] API Error: ${response.status}`);
        return [];
    }
    ...
    if (typeof __DEV__ !== 'undefined' && __DEV__) console.warn(`[Saavn] Search failed: ${getErrorMessage(error)}`);
    return [];
  }
}
...
export async function searchMusic(query: string, artistName?: string, onProgress?: (status: string) => void): Promise<UnifiedSong[]> {
  if (typeof __DEV__ !== 'undefined' && __DEV__) console.log(`[SearchEngine] 🚀 Searching JioSaavn. Query: "${query}"`);
  ...
  try {
      ...
      // FALLBACK TO GAANA
      if (results.length === 0) {
          if (typeof __DEV__ !== 'undefined' && __DEV__) console.log(`[SearchEngine] ⚠️ Saavn returned 0 results. Trying Gaana...`);
      ...
      if (message === 'TIMEOUT' || name === 'AbortError' || message.includes('Network request failed')) {
          if (typeof __DEV__ !== 'undefined' && __DEV__) console.warn(`[SearchEngine] Network timeout/error for "${query}".`);
      } else {
          if (typeof __DEV__ !== 'undefined' && __DEV__) console.error(`[SearchEngine] ⚠️ Search Failed:`, error);
      }
      return [];
  }
}
...
export async function getRecommendations(songId: string): Promise<UnifiedSong[]> {
  try {
    ...
    if (typeof __DEV__ !== 'undefined' && __DEV__) console.warn(`[SaavnRecs] Failed:`, error);
    return [];
  }
}
