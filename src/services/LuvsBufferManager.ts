...
class LuvsBufferManager {
  ...
  async enterLuvsMode() {
    ...
    if (typeof __DEV__ !== 'undefined' && __DEV__) console.log('[LuvsBuffer] Entering Luvs mode, setting up audio focus');
    ...
    try {
      ...
      if (typeof __DEV__ !== 'undefined' && __DEV__) console.log('[LuvsBuffer] Audio focus configured');
    } catch (error) {
      if (typeof __DEV__ !== 'undefined' && __DEV__) console.error('[LuvsBuffer] Failed to set audio mode:', error);
    }
  }
   ...
   */
  async exitLuvsMode() {
    if (typeof __DEV__ !== 'undefined' && __DEV__) console.log('[LuvsBuffer] Exiting Luvs mode, cleaning up');
      ...
      if (slot.sound) {
        try {
          ...
          if (!errorMsg.includes('Player does not exist')) {
            if (typeof __DEV__ !== 'undefined' && __DEV__) console.warn(`[LuvsBuffer] Unload failed for slot ${index}:`, errorMsg);
          }
        }
      }
    }
    ...
    try {
      ...
      if (typeof __DEV__ !== 'undefined' && __DEV__) console.log('[LuvsBuffer] Audio mode reset to default');
    } catch (error) {
      if (typeof __DEV__ !== 'undefined' && __DEV__) console.error('[LuvsBuffer] Failed to reset audio mode:', error);
    }
  }
  ...
  setSuspended(suspended: boolean) {
    if (typeof __DEV__ !== 'undefined' && __DEV__) console.log(`[LuvsBuffer] Suspension changed: ${this.isSuspended} → ${suspended}`);
  ...
  async updateActiveIndex(newIndex: number, feedSongs: UnifiedSong[], shouldPlay: boolean = true) {
    ...
    if (typeof __DEV__ !== 'undefined' && __DEV__) console.log(`[LuvsBuffer] Index: ${lastIndex} → ${newIndex}`);
    ...
    this.manageBuffer(newIndex, feedSongs).catch(e => {
        if (typeof __DEV__ !== 'undefined' && __DEV__) console.error('[LuvsBuffer] Buffer management failed:', e);
    });
  }
  ...
  private async playActiveSlot(index: number, feedSongs: UnifiedSong[], shouldPlay: boolean = true) {
    ...
    if (!this.slots.has(index)) {
      if (typeof __DEV__ !== 'undefined' && __DEV__) console.log(`[LuvsBuffer] Loading active slot ${index}`);
      await this.loadSlot(index, song);
    }
    
    // CRITICAL GUARD: Ensure we are STILL active after loading
    if (this.activeIndex !== index) {
        if (typeof __DEV__ !== 'undefined' && __DEV__) console.log(`[LuvsBuffer] 🛑 Aborted play for slot ${index} (Stale)`);
        return;
    }
      ...
      try {
        ...
        if (this.activeIndex === index) {
            ...
            if (shouldPlay) {
                ...
                if (typeof __DEV__ !== 'undefined' && __DEV__) console.log(`[LuvsBuffer] ▶️ Playing: ${song.title}`);
            } else {
                if (typeof __DEV__ !== 'undefined' && __DEV__) console.log(`[LuvsBuffer] ⏸️ Loaded but PAUSED: ${song.title}`);
            }
        }
      } catch (error) {
    ...
    const loadPromise = (async () => {
        try {
            ...
            if (this.loadingPromises.has(localTargetIndex) && isNeighbor) { 
                ...
                if (typeof __DEV__ !== 'undefined' && __DEV__) console.log(`[LuvsBuffer] ✅ Loaded slot ${localTargetIndex}`);
            } else {
