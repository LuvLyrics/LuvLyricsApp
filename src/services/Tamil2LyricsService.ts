...
class Tamil2LyricsService {
  ...
  async searchAndFetch(title: string, artist?: string): Promise<Tamil2LyricsResult | null> {
    try {
      ...
      if (typeof __DEV__ !== 'undefined' && __DEV__) console.error('[Tamil2Lyrics] Search failed:', error);
      return null;
    }
  }
  ...
  private async fetchLyrics(url: string): Promise<Tamil2LyricsResult | null> {
    try {
      ...
      if (typeof __DEV__ !== 'undefined' && __DEV__) console.error('[Tamil2Lyrics] Fetch failed:', error);
      return null;
    }
  }
