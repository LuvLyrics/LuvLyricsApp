...
class NativeSearchServiceImpl {
    ...
    async searchNativeYouTube(query: string): Promise<NativeSearchResult[]> {
        try {
            if (typeof __DEV__ !== 'undefined' && __DEV__) console.log(`[NativeSearch] Searching: ${query}`);
            ...
            if (!contents || !Array.isArray(contents)) {
                if (typeof __DEV__ !== 'undefined' && __DEV__) console.warn('[NativeSearch] No contents found in JSON tree');
                return [];
            }
            ...
            if (typeof __DEV__ !== 'undefined' && __DEV__) console.error('[NativeSearch] Failed:', error);
            return [];
        }
    }
