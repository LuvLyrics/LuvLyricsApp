...
export const MusicSearchService = {
  ...
  searchSongs: async (query: string): Promise<Song[]> => {
    try {
      ...
      if (typeof __DEV__ !== 'undefined' && __DEV__) console.error('[MusicSearchService] Search Error:', error);
      return [];
    }
  },
  ...
  getLyrics: async (id: string): Promise<Lyrics | null> => {
    try {
      ...
      if (typeof __DEV__ !== 'undefined' && __DEV__) console.error('[MusicSearchService] Lyrics Error:', error);
      return null;
    }
  }
};
