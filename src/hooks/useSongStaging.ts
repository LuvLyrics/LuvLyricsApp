...
export const useSongStaging = () => {
  ...
  const togglePreview = useCallback(async () => {
      ...
      try {
          ...
          if (typeof __DEV__ !== 'undefined' && __DEV__) console.warn('Preview failed', e);
      }
  ...
  const stageSong = useCallback(async (song: UnifiedSong) => {
    ...
    if (typeof __DEV__ !== 'undefined' && __DEV__) console.log(`[Staging] 🎨 Fetching iTunes cover art for: ${song.title} - ${song.artist}`);
    ...
    try {
        ...
        if (coverArtUrls.length === 0) {
            ...
            if (typeof __DEV__ !== 'undefined' && __DEV__) console.log(`[Staging] Raw search failed. Retrying with: ${cleanQuery}`);
            if (cleanQuery !== query) {
                 coverArtUrls = await ImageSearchService.searchItunes(cleanQuery);
            }
        }
        
        if (typeof __DEV__ !== 'undefined' && __DEV__) console.log(`[Staging] ✓ Got ${coverArtUrls.length} iTunes cover options`);
    } catch (e) {
        if (typeof __DEV__ !== 'undefined' && __DEV__) console.warn('[Staging] iTunes fetch failed, using source artwork', e);
    }
    ...
    setStaging({
        ...
        selectedCoverUri: selectedCover,
            ...
            if (typeof __DEV__ !== 'undefined' && __DEV__) console.log(`[Staging] ✓ Staged instantly with iTunes art: ${song.title}`);
            ...
            const fetchLyrics = async () => {
                try {
                    if (typeof __DEV__ !== 'undefined' && __DEV__) console.log('[Staging-V2] 🔄 Fetching lyrics in background (Multi-Source)...');
                    ...
                    if (typeof __DEV__ !== 'undefined' && __DEV__) console.log(`[Staging-V2] Lyrics found: ${lyricResults.length} options`);
                    ...
                    if (lyricResults.length > 0) {
                        setStaging(prev => {
                            ...
                            if (typeof __DEV__ !== 'undefined' && __DEV__) console.log('[Staging] ✓ Updating staging with lyrics');
                            return {
                                ...prev,
                        ...
                        if (typeof __DEV__ !== 'undefined' && __DEV__) console.log('[Staging] ⚠️ No lyrics found for this song');
                    ...
                    if (!isActive) return;
                    if (typeof __DEV__ !== 'undefined' && __DEV__) console.error('[Staging] ❌ Lyrics fetch failed:', e);
  ...
  const updateSelection = useCallback(async (updates: Partial<StagingSong>) => {
      if (typeof __DEV__ !== 'undefined' && __DEV__) console.log('[StagingHook] updateSelection called with:', updates);
      if (typeof __DEV__ !== 'undefined' && __DEV__) console.log('[StagingHook] Current staging:', staging);
      
      setStaging(prev => {
        const newStaging = prev ? ({ ...prev, ...updates }) : null;
        if (typeof __DEV__ !== 'undefined' && __DEV__) console.log('[StagingHook] New staging after update:', newStaging);
        return newStaging;
      });
  ...
  const finalizeDownload = useCallback(async () => {
    ...
    try {
      ...
      setStaging(prev => prev ? ({ ...prev, status: 'completed', progress: 1.0 }) : null);
       ...
       if (typeof __DEV__ !== 'undefined' && __DEV__) console.error('Download Failed:', error);
  ...
  const retryLyrics = useCallback(async () => {
    ...
    if (typeof __DEV__ !== 'undefined' && __DEV__) console.log('[Staging] 🔄 Retrying lyrics fetch...');
    setLyricFetchError(null); 
    ...
    try {
        ...
        if (typeof __DEV__ !== 'undefined' && __DEV__) console.log(`[Staging] Retry results: ${lyricResults.length}`);
        if (lyricResults.length === 0) {
            setLyricFetchError('No lyrics found for this song. Try title/artist edit and retry.');
        }
        ...
        if (typeof __DEV__ !== 'undefined' && __DEV__) console.error('[Staging] Retry failed:', e);
        setLyricFetchError(getLyricsFriendlyError(e)); 
